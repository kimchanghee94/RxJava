...
import com.example.rxjava.common.*;
...
public class FunctionalInterfaceToLamdaExample {
    public static void main(String[] args){
        ...
        List<Car> carsFilteredByPrice = CarFilter.filterCarByCustomPredicate(cars, car->car.getCarPrice()>30000000);

        for(Car car : carsFilteredByPrice){
            System.out.println("차 이름: " + car.getCarName() + ", 가격: " + car.getCarPrice());
        }

        List<Car> carsFilteredByCarType = CarFilter
                .filterCarByCustomPredicate(cars, car->car.getCarType().equals(CarType.SUV));
